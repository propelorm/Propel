...
";
     ...
     * @throws PropelException
     */
    public function get" . $this->getFKPhpNameAffix($fk, false) . "(PropelPDO \$con = null, \$doQuery = true)
    {";
        $script .= "
...
";
    ...
    {";
        ...
        \$query->joinWith('" . $this->getFKPhpNameAffix($fk2, false) . "', \$join_behavior);
...
";
     ...
     * @throws PropelException
     */
    public function get" . $this->getRefFKPhpNameAffix($refFK, false) . "(PropelPDO \$con = null)
    {
";
        $script .= "
...
";
    ...
    protected function addCrossFKDoAdd(&$script, ForeignKey $refFK, ForeignKey $crossFK)
    {
        ...
        $selfRelationNamePlural = $this->getFKPhpNameAffix($refFK, true);

        $lowerRelatedObjectClassName = lcfirst($relatedObjectClassName);

        $joinedTableObjectBuilder = $this->getNewObjectBuilder($refFK->getTable());
        $className = $joinedTableObjectBuilder->getObjectClassname();
        $refKObjectClassName = $this->getRefFKPhpNameAffix($refFK, false);
...
";
            ...
            foreach ($table->getForeignKeys() as $fk) {
                if ($fk->isLocalPrimaryKey()) {
                    $afx = $this->getFKPhpNameAffix($fk, false);
                    $script .= "
            \$relObj = \$this->get$afx();
            if (\$relObj) {
                \$copyObj->set$afx(\$relObj->copy(\$deepCopy));
